function [x, info] = ASQP_withfindingconstraints(Q, c, A, b, max_iter, toll, varargin)
    ...
    x0 = find_initial_feasible_point(A, b);
    if isempty(x0)
...
function x0 = find_initial_feasible_point(A, b)
    % Define the linear programming problem
    f = zeros(size(A, 2), 1);
    options = optimoptions('linprog', 'Display', 'none');
    
    % Solve the LP to find a feasible point
    [x0, ~, exitflag] = linprog(f, A, b, [], [], [], [], options);
