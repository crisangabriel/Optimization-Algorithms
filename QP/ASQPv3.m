function [x, info] = ASQPv3(Q, c, A, b, max_iter, toll, varargin)
    ...
    % Find an initial feasible point at the intersection of two constraints
    x0 = find_initial_feasible_point_intersection(A, b, toll);
    if isempty(x0)
        error('No feasible initial point found.');
    end
    fprintf('Initial feasible point found: [%.4f, %.4f]\n', x0(1), x0(2));
...
function x0 = find_initial_feasible_point_intersection(A, b, toll)
    % Find a feasible initial point at the intersection of two constraints
    n_constraints = size(A, 1);
    x0 = [];

    for i = 1:n_constraints-1
        for j = i+1:n_constraints
            Ai = A([i, j], :);
            bi = b([i, j]);
            if rank(Ai) == size(Ai, 1) % Check if Ai is full rank
                x_intersection = Ai \ bi;
                if all(A * x_intersection <= b + toll)
                    x0 = x_intersection;
                    return;
                end
            end
    ...
    % If no feasible intersection found, return empty
    if isempty(x0)
        x0 = [];
    end
end